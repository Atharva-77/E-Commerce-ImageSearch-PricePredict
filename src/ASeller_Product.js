...
function ASeller_Product() {
  ...
  const deleteUser = (id) => {
    ...
    if (window.confirm("Are you sure?")) {
      ...
      axios
        .delete(`${BASE_API_LINK}/products/seller/deleteProduct/${id}`, config)
        .then((res) => {
      ...
      axios
        .delete(`${BASE_API_LINK}/imgFeature/deleteFeature/${id}`, config)
        .then((res) => {
      ...
      axios
        .post(
          `https://e-commerce-imagesearch.vercel.app/delete_product`,
          flaskDeleteProd
        )
        .then((res) => {
  ...
  return (
    <div className="login">
      {/* {console.log();} */}
      {loading == true ||
      typeof Sellerproducts == "undefined" ||
      Object.keys(Sellerproducts).length == 0 ? (
        <div>
          <h2>Loading</h2>
        </div>
      ) : (
